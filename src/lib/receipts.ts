...
export interface ReceiptData {
  ...
  /**
   * Pre-rendered data: URL of the TSE QR code. Generated by the caller because
   * QR encoding is async and this function is not — see /api/receipts.
   */
  tseQrDataUrl?: string | null;
}
...
export function buildEscPosReceipt(
  ...
  return {
    ...
    // useful in text on an 80mm roll.
    tseLines:
      order.tse_status === "signed"
        ? [
            `TSE-Transaktion: ${order.tse_transaction_number ?? ""}`,
            `TSE-Signaturzaehler: ${order.tse_signature_counter ?? ""}`,
            `TSE-Seriennummer: ${order.tse_serial_number ?? ""}`,
          ]
        : undefined,
    tseFailed: order.tse_status === "failed",
...
export function generateReceiptHTML(data: ReceiptData): string {
  ...
  // KassenSichV §6 requires the TSE data on the receipt. The QR carries all of
  // it, so the plain-text fields are only a fallback for when there's no QR.
  // A failed signature must be visibly marked — an unsigned receipt that looks
  // signed is exactly what the rules are there to prevent.
  const tseBlock =
    order.tse_status === "signed"
      ? `<div class="divider"></div>
    <div class="tse">
      ${data.tseQrDataUrl ? `<div class="qr"><img src="${esc(data.tseQrDataUrl)}" alt="TSE"></div>` : ""}
      <div>TSE-Transaktion: ${esc(String(order.tse_transaction_number ?? ""))}</div>
      <div>TSE-Signaturzähler: ${esc(String(order.tse_signature_counter ?? ""))}</div>
      <div>TSE-Start: ${esc(order.tse_time_start ?? "")}</div>
      <div>TSE-Stop: ${esc(order.tse_time_end ?? "")}</div>
      <div>TSE-Seriennummer: ${esc(order.tse_serial_number ?? "")}</div>
      ${order.tse_client_serial ? `<div>Kassen-ID: ${esc(order.tse_client_serial)}</div>` : ""}
      ${order.tse_signature ? `<div class="sig">TSE-Signatur: ${esc(order.tse_signature)}</div>` : ""}
    </div>`
      : order.tse_status === "failed"
        ? `<div class="divider"></div>
    <div class="tse tse-fail"><strong>Hinweis: TSE-Ausfall — keine Signatur verfügbar.</strong></div>`
        : "";
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .tse { margin-top: 8px; font-size: 9px; line-height: 1.5; word-break: break-all; }
  .tse .sig { margin-top: 2px; }
  .tse-fail { font-size: 11px; }
  .qr { margin: 0 auto 6px; width: 42mm; height: 42mm; }
