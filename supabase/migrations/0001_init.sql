...
-- 1. ENUMS
-- ----------------------------------------------------------------------------
do $$ begin create type org_role as enum ('owner','admin','manager','staff','accountant','viewer'); exception when duplicate_object then null; end $$;
...
begin
  foreach t in array array['profiles','orgs','org_members','modules','member_module_access','invites',
    'vendors','inventory_items','inventory_transactions','recipes','recipe_ingredients',
...
declare t text;
begin
  foreach t in array array['vendors','inventory_items','inventory_transactions','recipes','recipe_ingredients',
    'purchase_orders','purchase_order_items','restaurant_tables','customers','orders','payments',
    'reservations','deliveries','employees','time_entries','tasks','loyalty_transactions',
...
declare t text;
begin
  -- member_module_access has a composite PK (org_id,user_id,module_id) and no `id`
  -- column, so write_audit()'s new.id reference fails. Never audit it; clear any
  -- trigger a prior migration created on it.
  drop trigger if exists audit_changes on public.member_module_access;
  foreach t in array array['recipes','inventory_items','orders','expenses','employees','purchase_orders'] loop
  ...
  begin
    for d in reverse 14..1 loop
      ...
      for i in 1..n loop
        ...
        _tip := case when random() > 0.5 then round((_subtotal * (0.1 + random()*0.1))::numeric, 2) else 0 end;
...
-- ----------------------------------------------------------------------------
-- 14. ROLE GRANTS
-- RLS policies above gate *which rows* each role sees, but PostgREST first needs
-- table-level privileges or every request fails with "permission denied for table".
-- Grant broadly to anon/authenticated and let RLS do the gating (Supabase convention).
-- ----------------------------------------------------------------------------
