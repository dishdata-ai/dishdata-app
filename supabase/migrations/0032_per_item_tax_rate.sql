-- ============================================================================
-- 0032 · Per-recipe VAT rate — food vs. drinks
-- German law taxes restaurant FOOD at the reduced rate (org.tax_rate, usually
-- 7%) but DRINKS at the standard rate (19%), even within the same order at
-- the same table. Until now every line in an order was taxed at one flat
-- org.tax_rate, which quietly under-collects VAT on every drink sold whenever
-- an org's default is the reduced food rate — exactly Kokoland's situation
-- today (org.tax_rate = 7, applied to Bier/Soft drinks/Lassi too).
--
-- recipes.tax_rate is nullable: null means "inherit the org's default rate"
-- (today's behavior, unchanged for anything not explicitly set), a number
-- overrides it per dish. The rate is snapshotted onto each order line at
-- checkout time (items[].tax_rate) — historical receipts must show the rate
-- that applied when the sale happened, not whatever the recipe is set to now.
--
-- Both checkout RPCs now extract VAT per rate group instead of once on the
-- whole order. Any discount is allocated proportionally across the groups by
-- their share of gross, so a discount doesn't quietly shift which rate
-- absorbs it. Signatures are unchanged (same params, same types), so
-- `create or replace` genuinely replaces rather than leaving a stale
-- overload — see 0019/0025 for why that distinction matters.
-- ============================================================================

alter table public.recipes add column if not exists tax_rate numeric;

-- ----------------------------------------------------------------------------
-- checkout_order (staff POS) — body from 0025, tax computation reworked.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- place_public_order (anon storefront) — body from 0031, tax computation
