-- ============================================================================
-- 0027 · Sold-out control for recipes
-- Managers can 86 an item for today (auto-clears at the stored cutoff, no cron
-- needed) or indefinitely (stored as a far-future timestamp). NULL = available.
-- The public storefront's place_public_order must reject sold-out items
-- server-side since it's callable by anon.
-- ============================================================================

alter table public.recipes add column if not exists sold_out_until timestamptz;

-- Re-declare place_public_order (10-arg overload from 0017_tax_included.sql)
-- with a sold-out check added to the item-validation loop. Everything else is
-- unchanged from 0017.
